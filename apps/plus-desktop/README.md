...
The Supervisor owns Harness Web, the configured Harness and Supervisor ports, the optional client HMR watcher, rebuilds, runtime logs, and its HTTP/SSE page. Windows starts the native Supervisor with Electron's utility process through an unpacked bootstrap, waits for the named pipe to finish listening, and returns the captured startup stack when initialization fails. A branch rebuild completes before the current Web process stops; a failed build leaves the current process running and retains raw output in the runtime log.
