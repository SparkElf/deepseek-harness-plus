...
Supervisor 拥有 Harness Web、配置的 Harness 与 Supervisor 端口、可选 client HMR watcher、rebuild、runtime log 和 HTTP/SSE page。Windows 使用 Electron utility process 启动 native Supervisor，并等待命名管道真正监听后，才报告 Supervisor 启动完成。branch rebuild 在停止当前 Web process 前完成；failed build 保留当前 process，并把 raw output 留在 runtime log。
