...
- **本地真实 API e2e**（`pnpm run test:e2e`）：由操作员手动运行并调用真实提供方 API。缺少对应提供方密钥时，各套件会自动跳过。GitHub Actions从不运行该命令，也不接收DeepSeek API凭据（[真实API e2e仅限本地运行的决策](../.agents/notes/implemented/testing/2026-08-23-real-api-e2e-local-only.md)）。
...
- **Web 浏览器快照**（`pnpm run test:web`）：Chromium 将回放语义与 `apps/web/tests/snapshots/` 比较。它断言可操作交互和可见状态，不断言样式；人审查视觉，截图只作证据。CI 通过 `DSH_SNAPSHOT=replay` 读取；record/refresh 留在本地（[web e2e 车道](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md)、[CI 门禁决策](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)）。`test:web` 会[先构建](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md)。
...
无密钥测试只能证明底层通路；本地带密钥运行可以证明agent（智能体）能对接真实模型正常工作。操作员手动运行的用例覆盖文件写入提示词、多轮对话、工具使用和流中取消，并通过真实示例及外部状态验证结果（[事故复盘0001](postmortem/0001-acp-default-export-drops-inject.md)）。自动跳过让无密钥贡献者不受阻塞。CI/CD保持keyless，且从不调用这些用例。
