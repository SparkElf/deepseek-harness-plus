...
- **Local real-API e2e** (`pnpm run test:e2e`): operator-run tests against live provider APIs. Each suite self-skips without its provider key. GitHub Actions never runs this command or receives DeepSeek API credentials ([local-only real-API e2e decision](../.agents/notes/implemented/testing/2026-08-23-real-api-e2e-local-only.md)).
...
- **Web browser snapshot** (`pnpm run test:web`): Chromium compares replayed semantics with `apps/web/tests/snapshots/`. It asserts operable interactions and visible state, not styling; humans review visuals, and screenshots are evidence only. CI reads via `DSH_SNAPSHOT=replay`; record/refresh stay local ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)). `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md).
...
## Pull request impact selection

Every pull request runs static checks after `scripts/resolve-ci-impact.mjs` classifies its merge-base diff. Documentation additions and edits skip unrelated jobs; reviewed Client mappings run changed Vitest coverage, one official build with validation and lint, and bounded mapped Web cases. Deletions, configuration, unmapped or non-Client source, mixed areas, and invalid input select the full matrix, as does the nightly default-branch run. The required aggregate accepts only planned skips. The [fail-open impact decision](../.agents/notes/implemented/testing/2026-08-22-fail-open-pr-ci-impact.md) owns details and mapping obligations.

## Local with-key policy

A no-key test proves plumbing; a local with-key run proves the agent works against a real model. Operator-run cases cover file-writing prompts, multi-turn conversations, tool use, and mid-stream cancellation against the real example and external state ([postmortem 0001](postmortem/0001-acp-default-export-drops-inject.md)). Self-skip keeps keyless contributors unblocked. CI/CD remains keyless and never invokes these cases.
