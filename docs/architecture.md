...
Plugin lifecycle, npm publication, source-repository ownership, and default profile composition are [independent decisions](../.agents/notes/implemented/architecture/2026-08-20-plugin-ownership-and-distribution.md). A first-party package may publish to npm, and a distribution may default-mount an external package without transferring its ownership.
