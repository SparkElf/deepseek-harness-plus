...
| `@deepseek-ai/dsh-tool-chart` | `render_chart` | `ctx.tools` | `tool/call`、`tool/result` | - | 紧凑canonical result把完整ECharts option保存在direct presentation metadata或nested `dsh/chart` content block中，用于durable browser replay。 |
...
从一个chart-ready resultRef准备的可JSON序列化ECharts option渲染一张交互式图表。option必须包含durable replay需要的全部图表数据。用DataOps SQL生成正确的业务粒度；Code Mode可在调用前进行面向可视化的映射、reshape、派生统计和标注。
...
来源：[`packages/chart/tool-chart/src/index.ts`](../packages/chart/tool-chart/src/index.ts)

紧凑canonical result把完整ECharts option保存在direct presentation metadata或nested `dsh/chart` content block中，用于durable browser replay。
