...
import type {
  SubagentEntryView, SubagentSettingsNamespace,
...
interface EntryEditorProps {
  ...
  writable: boolean
  saving: boolean
  t: (key: SubagentSettingsLocaleKey) => string
  onChange: (value: SubagentSettingsValue) => void
  onReset: () => void
}

function EntryEditor({ value: draft, writable, saving, t, onChange, onReset }: EntryEditorProps): ReactNode {
...
function descriptionKey(namespace: SubagentSettingsNamespace): SubagentSettingsLocaleKey {
  return namespace === 'subagent' ? 'subagentContinuousDescription' : 'subagentOneShotDescription'
}

function enabledKey(namespace: SubagentSettingsNamespace): SubagentSettingsLocaleKey {
        ...
        <div className={css.modeTabs} role="tablist" aria-label={t('subagentModes')}>
          {state.entries.map((candidate, index) => {
            ...
            return (
              ...
              >
                <span>{t(candidate.label as SubagentSettingsLocaleKey)}</span>
        ...
        >
          <div className={css.modeHeader}>
            <div className={css.modeCopy}>
              <h3>{t(entry.label as SubagentSettingsLocaleKey)}</h3>
