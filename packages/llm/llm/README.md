...
The attachment capability augments that map with `ImageBlock` and `DocumentBlock`. A durable document block requires one original reference plus one parsed bundle containing complete Markdown, content-list, and extracted-image references. Provider adapters resolve complete Markdown transiently for each request without mutating session history. They do not automatically inject extracted images in version one; every text-capable route still receives the complete Markdown.
