...
附件能力通过该 map 添加 `ImageBlock` 和 `DocumentBlock`。持久文档块必须包含一份原件引用，以及一份含完整 Markdown、content-list 和提取图片引用的解析产物。提供方适配器在每次请求时临时解析完整 Markdown，不改写会话历史。版本一不会自动注入提取图片；每条文本模型路由仍会收到完整 Markdown。
