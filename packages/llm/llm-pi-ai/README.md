...
An explicit `openai-responses` route may set `responsesCompatibility.omitReasoningInputStatus: true` for a gateway that accepts replayed reasoning items but rejects their response-only `status` field. The adapter applies this adjustment after pi-ai serializes the complete Responses request and before the SDK sends it: only top-level `type: reasoning` input items lose `status`; assistant messages and tool items keep their status metadata. The option defaults off, and enabling it is refused on every other protocol.
