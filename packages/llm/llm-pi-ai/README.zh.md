...
显式使用 `openai-responses` 的路由可以设置 `responsesCompatibility.omitReasoningInputStatus: true`，用于接受历史 reasoning 项、但拒绝其中响应侧专用 `status` 字段的网关。适配器在 pi-ai 完成整个 Responses 请求序列化之后、SDK 发出请求之前执行该调整：仅顶层 `type: reasoning` 输入项会失去 `status`，assistant message 与工具项的状态元数据保持不变。该选项默认关闭，在其他协议上启用会被拒绝。
...
转换保留逻辑请求顺序，不添加文本；复用取决于所选提供方的序列化与回放状态。更改适配器实例、提供方、模型或任何上游请求 token，都可能使复用从首个出现差异的 token 起失效。启用 reasoning 状态省略会改变历史 Responses JSON，但不增加 token，因此首次兼容请求可能无法复用由未调整协议元数据建立的前缀缓存；后续复用仍由提供方决定。
