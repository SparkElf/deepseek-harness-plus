...
// The prompt must equal the controlled prompt the dsh-terminal-bash backend
// matches after its OSC marker (CONTROLLED_PROMPT): any other PS1 leaves the
// backend's prompt readiness path unmatched, and every send falls back to the
// idleSilenceMs silence window.
