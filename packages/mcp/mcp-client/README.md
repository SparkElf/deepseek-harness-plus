...
- Reconnect states are user-visible in logs: reconnecting (warn, with attempt count and delay), recovered (info), final failure and disabled-loss (error). Disposal cancels any pending reconnect. It starts SDK close but uses the transport `onclose` signal and a five-second timeout as the quiescence decision, so a close Promise that remains pending after transport shutdown cannot wedge plugin teardown. With `reconnect.enabled: false`, a lost connection keeps tools registered but failing until a reload — the manual-recovery behavior.
