...
function provider(
  ...
  return { id, available: () => usable, parse }
}
...
describe('DocumentParserRuntime provider registration', () => {
  it('registers and disposes one provider', async () => {
    ...
    const dispose = parser.registerProvider(provider('mineru', true))
  ...
  it('rejects duplicate provider ids', async () => {
    const { parser } = await mountParser()
    parser.registerProvider(provider('mineru', true))
    expect(() => parser.registerProvider(provider('mineru', true))).toThrow(expect.objectContaining({
  ...
  it('removes provider registrations when the contributing fiber is disposed', async () => {
    ...
    const fiber = await ctx.plugin(Object.assign((inner: Context) => {
      inner.documentParser.registerProvider(provider('mineru', true))
...
describe('DocumentParserRuntime provider selection', () => {
  it('fails when no usable provider exists', async () => {
    const { parser } = await mountParser()
    parser.registerProvider(provider('mineru', false))
    await expect(parser.parse(request)).rejects.toThrow(expect.objectContaining({
      code: 'DOCUMENT_PARSER_UNAVAILABLE',
    }))
  })

  it('fails rather than selecting by registration order when multiple providers are usable', async () => {
    const { parser } = await mountParser()
    parser.registerProvider(provider('mineru', true))
    parser.registerProvider(provider('other', true))
    await expect(parser.parse(request)).rejects.toThrow(expect.objectContaining({
  ...
  it('fails when the configured provider is unavailable', async () => {
    ...
    parser.registerProvider(provider('mineru', false))
    await expect(parser.parse(request)).rejects.toThrow(expect.objectContaining({
      code: 'DOCUMENT_PARSER_CONFIGURED_UNAVAILABLE',
    }))
  })

  it('uses the configured provider when another provider is also usable', async () => {
    const { parser } = await mountParser('mineru')
    parser.registerProvider(provider('other', true, () => Promise.reject(new Error('wrong provider'))))
    parser.registerProvider(provider('mineru', true))
  ...
  it('forwards the request and AbortSignal to the selected provider', async () => {
    ...
    parser.registerProvider(provider('mineru', true, (input, signal) => {
