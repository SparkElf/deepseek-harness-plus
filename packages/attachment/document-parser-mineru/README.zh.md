...
Harness [`documentParser`](../document-parser/README.md) 能力的 MinerU HTTP 实现。它注册提供方 id `mineru`，并调用一个由部署方管理的 MinerU 同步 `/file_parse` 端点。Node 包不内嵌 Python、PyTorch、模型权重、GPU 生命周期或 MinerU 进程管理。
...
[base bundle](../../bundle/base/cordis.patch.yml) 安装了默认禁用的 `document-parser` 与 `document-parser-mineru` 两行。部署方在后续 patch layer 中成对启用它们，并按需修改这些显式值；只启用 Definition 时不会发布文档选择器能力。
...
MinerU 的完整 Markdown 是从持久引用解析后送给文本模型的提供方中立表示。`content_list` 与提取图片保持持久，可供文档工具使用，但不会自动注入模型请求。

#### Token 影响

包含文档消息的每次请求都会计入完整渲染 Markdown 的 token。MinerU 不会把解析元数据、`content_list`、提取图片或 HTTP/ZIP 细节加入请求。
