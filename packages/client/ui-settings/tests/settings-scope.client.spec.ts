...
function derivedScope(
  api: { describe?: ReturnType<typeof vi.fn>; mutate?: ReturnType<typeof vi.fn>; replace?: ReturnType<typeof vi.fn> },
...
describe('SettingsScopeController', () => {
  ...
  it('replaces one complete user section under the current revision and folds the answer', async () => {
    const describeCall = vi.fn().mockResolvedValueOnce(described({ preference: 'system' }, 4))
    const replace = vi.fn().mockResolvedValueOnce(ok(view({ preference: 'dark' }, 5)))
    const { mirror, scope } = derivedScope({ describe: describeCall, replace })
    await mirror.load()
