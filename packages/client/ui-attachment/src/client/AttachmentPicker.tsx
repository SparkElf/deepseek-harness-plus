...
import { IconPaperclipOutline16, IconWarningOutline16, Toast } from '@deepseek-ai/dsh-client-ui-primitives'
...
/** Native picker filter for image input plus parser-backed version-one document types. */
...
export interface AttachmentPickerInjected {
  ...
  readonly draftStats: (ids: readonly DraftAttachmentId[]) => DraftAttachmentStats
  /** Register one already-prechecked mixed file list and preserve its original order. */
  readonly createDrafts: (files: readonly File[]) => readonly DraftAttachmentId[]
  /** Release every id created by a picker transaction that the input machine refused. */
  readonly releaseDrafts: (ids: readonly DraftAttachmentId[]) => void
}
...
export function AttachmentPicker({
  ...
  const validate = (files: readonly File[]): string | null => {
    ...
    if (documents.length > 0 && documentLimits === undefined) return t('document.unavailable')
  ...
  const onFiles = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    try {
      ...
      console.error('ui-attachment: draft creation failed', reason)
  ...
  return (
    <>
      <input
        ...
        type="file"
        multiple
        accept={documentLimits === undefined
          ? IMAGE_ACCEPT
          : [IMAGE_ACCEPT, ...documentLimits.mediaTypes].join(',')}
        tabIndex={-1}
        aria-hidden="true"
        onChange={onFiles}
      />
