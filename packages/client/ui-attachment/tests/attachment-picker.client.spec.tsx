...
import {
  ATTACHMENT_ACCEPT, AttachmentPicker, type AttachmentPickerProps,
} from '../src/client/AttachmentPicker.tsx'
...
const t = ((key: string, params?: Readonly<Record<string, unknown>>): string => {
  ...
  if (key === 'image.tooMany') return `最多 ${String(params?.count ?? '')} 张图片`
  if (key === 'document.tooMany') return `最多 ${String(params?.count ?? '')} 个文档`
  return key
...
describe('AttachmentPicker', () => {
  it('advertises the complete PR A picker vocabulary', () => {
    ...
    expect(ATTACHMENT_ACCEPT).toContain('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    expect(ATTACHMENT_ACCEPT).toContain('application/vnd.openxmlformats-officedocument.presentationml.presentation')
    expect(ATTACHMENT_ACCEPT).toContain('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  })
