...
`ComposerAttachments` listens to the document-level file drag owned by this slot entry. File drops are split by kind: raster images are forwarded to the existing image intake callback; document files are offered only while the Host projects parser-backed `documentLimits`, then forwarded to the document intake callback whose owner performs PDF/DOCX/PPTX/XLSX prechecks. A blocked composer shows the generic attachment-disabled overlay and forwards nothing. The overlay is presentation only; count/byte/media-type admission stays in the conversation owner and Host.
...
- Historical document cards intentionally remain metadata-only; parsed Markdown reaches the model through the LLM adapter, and parser internals do not belong in this package.
