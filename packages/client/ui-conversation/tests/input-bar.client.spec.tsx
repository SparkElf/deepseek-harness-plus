...
function bench(over?: BenchOptions) {
  ...
  const shell = new SessionInputShell({
    actx: SCTX,
    defaultSink: sink,
    commandImages: { selectIds: ids => ids, serialize: () => Promise.resolve([]), release: () => {}, unsupportedNotice: (token: string) => `${token.trim()} images-unsupported`, unsupportedDocumentNotice: (token: string) => `${token.trim()} documents-unsupported` },
    queue: {
...
describe('strips and variants', () => {
  ...
  it('renders slot items with the left action between the command launcher and mode controls', () => {
    const { view } = bench({
    ...
    const command = view.getByLabelText('命令')
    const left = view.getByTestId('li')
    const access = view.getByLabelText(/^访问模式/)
    expect(command.compareDocumentPosition(left) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)
    expect(left.compareDocumentPosition(access) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)
...
describe('command launcher chrome and control seats', () => {
  ...
  it('the Access chip renders the projection value and submits a non-Full-access pick directly', async () => {
    ...
    // Localized display is presentation only; the menu ids stay machine names.
  ...
  it('requires explicit risk acknowledgement before submitting localized full access', async () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
  ...
  it('cancels a localized full-access selection without changing permission and resets acknowledgement', () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
