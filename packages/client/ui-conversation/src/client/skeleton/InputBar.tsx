...
export function InputBar({
  ...
  // The deployment's attachment-intake limits (absent while no attachment
  // service is composed — the pre-check below then defers entirely to Host).
  ...
  // The input machine predates documents and names this ordered opaque list
  // `imageIds`. Keep every registry-backed id, regardless of kind, and prune
  // only records whose runtime draft object disappeared.
  useEffect(() => {
    ...
    const rejected = ((): string | null => {
      if (documentLimits !== undefined) {
        ...
        if (documents.length + files.length > documentLimits.maxDocumentsPerMessage) {
          return `A message can include up to ${documentLimits.maxDocumentsPerMessage} documents`
        }
        if (files.some(file => file.size > documentLimits.maxDocumentBytes)) {
          return `Each document must be smaller than ${imageSizeText(documentLimits.maxDocumentBytes)}`
        }
        const total = documents.reduce((sum, attachment) => sum + attachment.file.size, 0)
          + files.reduce((sum, file) => sum + file.size, 0)
        if (total > documentLimits.maxMessageDocumentBytes) {
          return `Documents exceed ${imageSizeText(documentLimits.maxMessageDocumentBytes)} in total; remove some and try again`
        }
      }
      return addDocuments(files)
    })()
    if (rejected !== null) showToast(rejected)
  }, [addDocuments, documentLimits, documents, showToast])
