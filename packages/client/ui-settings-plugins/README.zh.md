...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、DeepSeek 搜索提供方（`web-search-deepseek`），以及两个已发布的子 agent 委派入口（`subagent` 和 `subagent-fork`）。
...
保存时，标量卡片通过客户端 settings scope 写入每个暂存字段，每张子 agent 卡片则通过自己的命名空间 scope 替换完整暂存入口。两条路径都用读取时的命名空间 revision 为写入设栅，因此已与文档脱节的表单会被拒绝，而不是覆盖并发变更。scope 会把已提交的应答折入共享 Settings 镜像；被拒绝的替换会报告保存未落盘，并保留草稿供用户修改。
