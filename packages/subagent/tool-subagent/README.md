...
Settings lifetime is independent from Agent lifetime. A Host composition mounts `@deepseek-ai/dsh-tool-subagent/settings` once per namespace; any number of Agent-preset tool instances consume that registered section through `settingsNamespace`. Without a Settings service the tool uses its own composition defaults. When a Settings service exists, naming an unregistered namespace fails the tool mount instead of creating an Agent-lifetime registration.
...
| `settingsNamespace` | Optional Host-registered namespace for live user-owned child defaults. Each later child start reads its resolved `agentOptions`, `persona`, `toolFilter`, and `maxDepth`; mount the `/settings` subpath once at Host lifetime to own it. |
