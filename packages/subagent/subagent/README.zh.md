...
子 agent 的初始 provider、model、推理强度和输出上限，优先从委派父级最近一次已记录的 `request/header` 解析；父级尚未发起请求时才使用父级的 `AgentOptions`。每个子级请求显式给出的覆盖值仍然优先。这个路由会在创建前确定，因此新的子 agent 会使用父会话选中的模型，而不会回退到部署默认值。
