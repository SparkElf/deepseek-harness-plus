...
`openNativePath(path, signal)` hands a path to the default application and prefers the named default browser for HTML and SVG where the platform can identify one. `openNativeTextFile(path, signal)` selects text-editor intent; on macOS it uses `open -t`. WSL paths are translated with `wslpath -w` before the Windows desktop receives them, and the interpreter is named on the mounted Windows volume when PATH carries no Windows entry — a service started outside WSL's interactive PATH injection still reaches the desktop. `canOpenNativePath()` reports whether the current Host plausibly has a desktop target; `canOpenLinuxDesktop()` narrows that to a Linux desktop program, which WSL never has because it opens through Windows. A caller that offers the Linux file manager asks the narrower question and, on a WSL host, resolves to the Windows desktop instead of dropping the entry.
