...
Web Session 日志下载控制，使用 `dsh-host-apiproxy` 拥有的 Host 流式 ZIP 端点。Host 半包注册 `/export`；浏览器半包提供一个供 Header 按钮与斜杠命令共用的下载控制器和弹窗。Header 操作在桌面端是 111×32 的胶囊，随 locale 显示 `Session log` 或“会话日志”；在手机端则是保留同一本地化无障碍名称的无边框 28×28 下载图标。Better Sidebar 折叠时，该图标沿用该 shell 的固定右上控件节奏，位于展开按钮左侧 4px。ZIP 生成、原始 JSONL/zstd 读取、子 Session、附件、背压和 HTTP 错误语义仍由 [ApiProxy 下载实现](../../host/apiproxy/README.md)负责。
