...
Web Session-log download control over the host-streamed ZIP endpoint owned by `dsh-host-apiproxy`. The Host half registers `/export`; the browser half owns one download controller and one modal shared by the Header button and slash command. The Header action is a 111×32 capsule on desktop whose locale label is `Session log` or `会话日志`, and a borderless 28×28 download icon with the same localized accessible name on phones. While Better Sidebar is collapsed, the icon follows that shell's fixed top-right control rhythm and sits 4px left of the expand button. ZIP generation, raw JSONL/zstd reads, descendants, attachments, backpressure, and HTTP error semantics remain owned by the [ApiProxy download implementation](../../host/apiproxy/README.md).
