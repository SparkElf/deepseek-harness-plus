...
import LlmRuntime, { LlmAdapter, renderParsedDocumentText } from '@deepseek-ai/dsh-llm'
...
const renderedDocumentBytes = (name: string, markdown: Uint8Array): number => new TextEncoder().encode(
  renderParsedDocumentText({
    attachmentId: AttachmentId('budget-ref'),
...
describe('session.prompt document parsing admission', () => {
  it('persists parser outputs and accepts the exact complete rendered-text budget', async () => {
    const markdown = new TextEncoder().encode('# parsed')
    const state = await harness({ maxDirectMarkdownBytes: renderedDocumentBytes('sample.pdf', markdown) })
  ...
  it('rejects document submission when no parser is composed and appends no user message', async () => {
    const state = await harness({ withParser: false })
    const response = await apiFor(state.ctx).sessions.prompt(request({
      sessionId: state.sessionId,
      mode: 'queue' as const,
    ...
    expect(response.result).toMatchObject({
      ok: false,
      error: { code: 'attachment-error', details: { reason: 'DOCUMENT_PARSER_UNAVAILABLE' } },
    })
  ...
  it('rejects over-budget complete rendered document text before appending the user message', async () => {
    const markdown = new TextEncoder().encode('12345')
    const state = await harness({
      maxDirectMarkdownBytes: renderedDocumentBytes('sample.pdf', markdown) - 1,
  ...
  it('enforces the complete rendered-text byte budget across every document in one submitted message', async () => {
    const markdown = new TextEncoder().encode('abc')
    const state = await harness({
      maxDirectMarkdownBytes: renderedDocumentBytes('one.pdf', markdown) + renderedDocumentBytes('two.pdf', markdown) - 1,
      parse: () => Promise.resolve({
        parser: 'mineru',
        result: { markdown, contentList: new TextEncoder().encode('[]'), images: [] },
      }),
