...
import { parseDocumentRefs, type ParsedDocument } from './document-parsing.ts'
...
async function durablePromptContent(ctx: Context, content: readonly PromptContentPart[]): Promise<ContentBlock[]> {
  ...
  const parsedDocuments = await parseDocumentRefs(ctx, documentRefs)
  let imageIndex = 0
  let documentIndex = 0
  return content.map((part): ContentBlock => {
    switch (part.type) {
      ...
      case 'document': {
        const document = parsedDocuments[documentIndex++] as ParsedDocument
        return { type: 'document', attachment: document.attachment, parsed: document.parsed }
      }
    }
  })
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  ctx.inject(['sessionProjections', 'attachments', 'documentParser'], (projectionCtx) => {
    // Publish picker policy only when current registry state resolves a parser selection.
    if (projectionCtx.attachments.documentLimits.mediaTypes.length === 0) return
    if (!projectionCtx.documentParser.isSelectionResolvable()) return
      ...
      async prompt(request) {
        ...
        const admit = async (): Promise<RpcResponse<{ accepted: true }>> => {
          try {
            ...
            ctx.logger.error('apiproxy: prompt admission failed')
            ctx.logger.error(error)
            if (error instanceof AttachmentError || error instanceof DocumentParserError) {
              return err(request, {
                code: 'attachment-error',
                message: error.message,
