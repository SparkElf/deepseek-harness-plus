...
rc15 explicit apply步骤固定official revision `0a53fb55bea101816fa226bb964ae2bed71c343b`，一次应用全部pending source payloads，配置exact npm patches，并以`0600` mode写入`.dsh-plus/patchset.lock.json`。profile中的patched official source workspace始终覆盖同版本clean CLI package。mobile layout patch正式拥有Settings、Model Selection和Conversation chrome，因此从固定official base重建时不会再丢失后续responsive UI。profile还通过精确版本或正式release assets拥有Better Sidebar 0.18.1、Univer 0.2.14、政府公文、SQL Workbench、共享vault、SSH Manager、API Client和Supervisor 0.1.2。version conflict、source mismatch、bundle缺失和runtime closure drift都会阻止promotion。
