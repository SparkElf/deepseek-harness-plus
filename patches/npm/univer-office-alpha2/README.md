...
This data-only npm package directly patches the exact `dsh-univer-office@0.2.12` artifact. It preserves the existing DSH alpha.2 Settings, Tool-input, peer-version, and Viewer socket-lifecycle compatibility changes. It also backports `univer_new.templateFile`, complete non-overwriting `.univer` copies, and reversible read-only template-root registration for trusted Host plugins; output targets remain session-workspace confined, while registered external assets are staged inside that workspace before Gateway creation.
