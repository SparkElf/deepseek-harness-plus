...
/** Resolve the repository targeted by this run; GitHub Actions owns the live repository fact. */
export function resolveRepositoryCoordinates(slug, defaults) {
  if (slug === undefined || slug === '') return defaults
  const parts = slug.split('/')
  if (parts.length !== 2 || parts.some((part) => part === '')) {
    throw new Error(`GITHUB_REPOSITORY 必须是 owner/repository，收到 ${JSON.stringify(slug)}`)
  }
  return { organization: parts[0], repository: parts[1] }
}

const repositoryCoordinates = resolveRepositoryCoordinates(process.env.GITHUB_REPOSITORY, {
...
async function issueSnapshot(number, status = undefined) {
  const issue = await api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/issues/${number}`)
  if (issue.pull_request) return null
  const values = await api(
    `/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/issues/${number}/issue-field-values?per_page=100`,
  )
...
async function projectContext(number, includeStatusActor = false) {
  const data = await graphql(
    ...
    {
      organization: repositoryCoordinates.organization,
...
async function upsertAudit(number, errors) {
  const comments = await api(
    `/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/issues/${number}/comments?per_page=100`,
  )
  ...
  if (errors.length === 0) {
    if (existing) {
      await api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/issues/comments/${existing.id}`, {
        method: 'DELETE',
      })
    }
    return
  ...
  if (existing) {
    ...
    await api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/issues/${number}/comments`, {
      method: 'POST',
      body: JSON.stringify({ body }),
      headers: { 'Content-Type': 'application/json' },
    })
...
async function resolvingReferencesSnapshot(number, pull) {
  const references = parseReferences({
    body: pull.body ?? '',
    repository: `${repositoryCoordinates.organization}/${repositoryCoordinates.repository}`,
  })
...
async function pullRequestSnapshot(number) {
  const [pull, reviewRequests, reviews] = await Promise.all([
    api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/pulls/${number}`),
    api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/pulls/${number}/requested_reviewers`),
    api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/pulls/${number}/reviews?per_page=100`),
  ])
...
async function lifecyclePullRequestSnapshot(number) {
  const pull = await api(`/repos/${repositoryCoordinates.organization}/${repositoryCoordinates.repository}/pulls/${number}`)
