...
- **Integrations stay in plugins.** Keep integration-specific behavior in optional plugins or overlays; core edits require prior gap, impact, and alternatives reporting plus explicit user approval ([decision](.agents/notes/implemented/architecture/2026-08-20-plugin-ownership-and-distribution.md)). `agent-loop` changes also update docs/architecture.md.
- **Integration security follows declared trust and user convenience.** Respect trusted HTTP/internal and HTTPS/external without fallback; absent product or protocol need, reject rotation, refresh, locks, queues, polling, reauthorization, or remounts ([decision](.agents/notes/implemented/architecture/2026-08-20-plugin-ownership-and-distribution.md)).
...
- **Testing policy** — [docs/testing.md](docs/testing.md). Automate operable GUI behavior and visible state; humans review visual styling. Non-trivial model- or user-visible behavior changes add or update a keyless runnable-example snapshot in the same PR. Fixtures replay on macOS/Linux; fix fixtures, not normalizers.
