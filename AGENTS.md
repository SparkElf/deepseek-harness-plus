...
- **Plugins own integrations; security follows trust and convenience.** Core edits need explicit approval. Respect trusted HTTP/internal and HTTPS/external without fallback; absent product/protocol need, reject rotation, refresh, locks, queues, polling, reauthorization, or remounts ([decision](.agents/notes/implemented/architecture/2026-08-20-plugin-ownership-and-distribution.md)).
