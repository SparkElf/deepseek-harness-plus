...
- **Integrations stay in plugins.** Core changes require gap, impact, and alternatives reporting plus explicit approval. Honor declared internal-HTTP/external-HTTPS trust; absent product or protocol need, reject added security lifecycle machinery ([decision](.agents/notes/implemented/architecture/2026-08-20-plugin-ownership-and-distribution.md)).
...
- **Both SDKs project the loop.** Agent-loop, session-lifecycle, and `SessionEventMap` changes update `docs/architecture.md` plus the TypeScript and Python SDK expected outputs in the same PR; `pnpm run test` covers neither ([surfaces](docs/testing.md#when-a-snapshot-test-is-required)).
