...
describe('dependency sections', () => {
  ...
  it('accepts published SparkElf minimum ranges with workspace development mirrors', () => {
    const manifest: PackageDependencyManifest = {
      ...
      peerDependencies: {
        '@deepseek-ai/dsh-runtime': '>=0.1.2-alpha.2',
        [CORDIS]: '>=4.0.1',
      },
    }
    const base = facts(manifest)
    ...
    repairPackageDependencyManifest(subject)
    expect(manifest.peerDependencies?.['@deepseek-ai/dsh-runtime']).toBe('>=0.1.2-alpha.2')
    expect(manifest.devDependencies?.['@deepseek-ai/dsh-runtime']).toBe('workspace:^')
