...
Plus 将为 macOS、Linux 和 Windows 提供一个常驻托盘的 Electron 管理器。本地 runtime 不存在时，它会打开一次性单栏安装引导，并复用 Harness design token 与品牌资产。语言和主题选择会立即改变引导，并成为已安装 Harness 的默认设置。引导还会收集空安装目录、本地端口、DeepSeek key、默认模型和可选推理强度。

Windows 用户可以选择 Windows 文件系统或一个已安装的 WSL 发行版。所选 target 拥有 checkout、DSH_HOME、settings、Git 与 pnpm 命令、Supervisor、Harness Web process、rebuild、repair 和 update。Linux 与 macOS 使用本机环境。引导在该 target 中 clone 并构建 Plus，启动 Harness Web，打开浏览器页面，然后关闭。
...
所选 target 需要安装 Git 和 pnpm。WSL 安装还需要一个已经安装这些工具的发行版。引导会在安装时写入初始 DeepSeek key，因此它的 preload bridge 只保留安装 IPC，并会在成功后关闭。跨平台安装器和原生桌面流程验证在匹配 runner 产出并验证 artifact 前仍未交付。升级只允许快进 Git 更新；修改过的 checkout 如何恢复需要一项单独且明确的用户决策。
