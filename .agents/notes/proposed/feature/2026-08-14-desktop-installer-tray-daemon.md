...
A person uses DeepSeek Harness through its browser page, but the first local runtime needs an installation path, one provider credential, and a default model before that page can open. Once installed, a second desktop settings surface would duplicate the Web page and create conflicting owners for models, credentials, presets, and workspace selection.

## Proposal

Plus will provide a tray-resident Electron manager for macOS, Linux, and Windows. When no local runtime exists, it opens a one-time compact four-step setup guide that uses the Harness design tokens and brand assets. Language and theme selections change the guide immediately and become the installed Harness defaults. The guide collects an empty installation directory, one default provider, its key, model, and optional reasoning effort; its advanced port choice defaults to 3080. It supports the installed key-based catalog providers and a custom OpenAI-compatible provider, then hands later provider and model management to Harness Web.

Windows users choose either the Windows filesystem or one installed WSL distribution, then select an empty folder through the native file picker. WSL folder selection opens the selected distribution in Explorer and converts its result to the distribution path. The selected target owns the checkout, DSH_HOME, settings, Git and pnpm commands, Supervisor, Harness Web process, rebuilds, repairs, and updates. Linux and macOS use their native environment. The guide clones and builds Plus in that target, starts Harness Web, opens the browser page, and closes.
...
- An initial four-step guide captures an installation path, one supported catalog or custom OpenAI-compatible provider, its managed credential, default model, and reasoning effort before the first service starts; port selection remains an advanced option.
