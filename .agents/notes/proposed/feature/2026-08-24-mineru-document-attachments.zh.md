...
现有 `$DSH_HOME/attachments/v1/objects` content-addressed store 继续作为物理对象存储。local provider 只增加文档与 parser artifact 所需的最小 generic byte-file primitive，而不是再创建 MinerU 专属目录树。原始文件名只作为 display metadata，永远不参与 storage path。
