...
| Optional DataOps integration in DSH | `@sparkelf/dsh-dataops-integration` in `dsh-plugins-plus` | Owns Authorization Code + PKCE, the delegated access credential, Settings UI, routes, and generic MCP composition; omission leaves generic MCP transport unauthenticated |
| Generic MCP transport | DSH MCP client | Uses ordinary Streamable HTTP headers exactly as configured and remains unaware of DataOps identity, OAuth, credentials, and lifecycle |
...
| Batch AI analysis | `@sparkelf/dsh-query-result-analysis` plus DSH's built-in session-backed in-process `spawn` provider | The plugin registers `analyze_query_result` and owns paging, checkpoints, and reduction; durable child sessions and their AgentLoop own model calls, logging, and retry |
...
A DataOps-managed container does not repeat browser OAuth. The authenticated DataOps browser reaches DSH only through the Workspace Web Gateway at `http://<dshTargetRef>.dsh.<internal-domain>/`: one immutable managed target owns one browser origin, while static wildcard DNS routes every target host to the same gateway. The DataOps parent and target hosts use the same scheme and site. The trusted-internal deployment contract defaults to HTTP/WS and a host-only HttpOnly SameSite=Strict cookie without `Secure`; it never falls back from an explicitly configured HTTPS parent to HTTP. A one-time launch code binds the target host, user, and active DataOps session before the gateway sets that cookie. Each user's main container owns an independent network namespace, and only its companion joins and listens on loopback port 3080, so users share that port without host publication. The existing workspace agent tunnel forwards native root-mounted DSH HTTP and WebSocket traffic to that loopback service; DSH core needs no base-path support. Inside the container, DSH calls the local MCP adapter, which delegates through a DataOps-owned Unix broker. Only the broker receives the workspace user ID, internal token, and backend location; the DSH main process, model context, URL, and tool arguments do not receive those credentials. Managed DSH does not mount the standalone authorization plugin, so Settings has no Connect, Reauthorize, or Disconnect actions.

The external `@sparkelf/dsh-dataops-integration` plugin is optional for standalone DSH. It reads its delegated access credential and mounts the generic Streamable HTTP MCP client with an ordinary `Authorization` header. If the plugin is absent, a directly configured generic MCP client sends only its explicit headers and may attempt an anonymous connection; the remote MCP server owns whether anonymous access exists. The production DataOps data-query MCP endpoint requires authentication and rejects that anonymous attempt.

A direct standalone DSH visit starts DataOps Authorization Code with PKCE. Loopback HTTP callbacks need no registration; every non-loopback HTTP or HTTPS callback origin is explicitly registered, and HTTP denotes a trusted-LAN contract rather than an HTTPS fallback. DSH stores one random target reference for each standalone connection. DataOps owns the browser authorization page, normal login and MFA, and explicit account confirmation. The first approval atomically binds an unbound target reference to the selected OIDC `sub`; every later approval and MCP session for that active connection must use the same owner. One delegated access token remains valid only while its bound DataOps `AuthSession`, account, target owner, audience, scope, and current permissions remain valid. Explicit Disconnect verifies and revokes the grant, releases the standalone binding, and rotates a writable target, so the next Connect returns to explicit account selection. DataOps-managed workspace targets remain immutable and expose no standalone connection actions.

A launch from DataOps may carry only a short-lived, single-use code bound to the user, OAuth client, target DSH instance, audience, state, and expiry; the plugin exchanges it server-side and removes it from the URL. The delegated access credential never appears in URL parameters, tool arguments, model context, browser local storage, or session logs. DataOps Web access tokens and refresh cookies remain tab-isolated; a new reauthorization popup recognizes existing active browser sessions through the signed authorization request and automatically carried HttpOnly cookies instead of copying a prior popup's token or auth-tab identity. The plugin stores the delegated credential through the DSH credential service and supplies it to the generic MCP child's ordinary HTTP header configuration. Explicit reauthorization replaces the credential and remounts that one child once; there is no background refresh, periodic rotation, or periodic remount.

Authenticated Streamable HTTP MCP requests carry `Authorization: Bearer <access-token>`. DataOps validates issuer, audience, bound and still-active `AuthSession`, authorized client, user, target owner, and MCP scope, then projects an `AuthorizationPrincipal` into the tool handler. The MCP audience and scope permit entry to the protected endpoint but grant no Resource access. Existing DataOps permissions and Resource authorization decide each tool operation, and result pages and exports reauthorize the current user. DSH does not copy or interpret the DataOps role and permission matrix.

DataOps binds each connected MCP session identifier to one runtime principal. An explicitly reauthorized credential for the same OIDC `sub` may replace the prior grant and remount the MCP child once; a different `sub` is rejected while that connection exists. Explicit Disconnect ends the standalone connection before a new target can select another account. MCP session identifiers, DSH conversation identifiers, and result references are not credentials. A conversation identifier may later provide audit correlation without changing authorization.
...
MCP arguments and `structuredContent` use JSON. Model-facing text may be compact, but it must not duplicate the complete structured payload. Cross-boundary identifiers are opaque and principal-scoped. Access tokens travel only in the HTTP `Authorization` header; tool arguments never contain access tokens, user IDs, tenant IDs, or verification fields. Models never receive database credentials, connection strings, or unscoped internal numeric IDs.
...
`analyze_query_result({resultRef, instruction, resumeAnalysisRef?})` is an AI semantic-analysis workflow over a result that DataOps has already filtered, joined, aggregated, and sorted to the required business grain. Each DataOps page becomes one model-only DSH child session with stable `<resultRef>#row-N` evidence labels; reduction groups run in the same kind of durable child. The plugin checkpoints completed batches, propagates cancellation, and reduces summaries until it returns `analysisRef`, `summary`, row and batch counts, resume state, and every analysis child session id. Each child inherits the active provider/model configuration, and its normal AgentLoop persists complete model-visible input and output while applying the provider retry policy. Operation checkpoints remain plugin files rather than root-session events, so this optional package requires no DSH core event type or reader dependency.
...
The Agent may call `render_chart` directly for a simple chart in Native or Both mode. In Code Mode, the program reads the required result pages, performs presentation-oriented mapping, filtering, sorting, reshape, type conversion, percentages, cumulative values, reference lines, or annotations, and invokes `render_chart` as a nested SDK dispatch with the completed JSON ECharts option. Code Mode does not join multiple result references.

The `option` contains all data required for replay and must be JSON-serializable; JavaScript callback functions are excluded. A direct result stores the complete option in tool-result `presentationMeta`; a nested Code Mode result stores the same validated projection in a chart-owned block inside durable `tool/code-dispatch.content`, which never re-enters model context. `sourceResultRef` remains provenance only. Session replay renders from the recorded option and does not contact DataOps or depend on the result TTL.
...
DataOps `AiWorkspaceService` owns the one-user-one-container mapping, independent `DSH_HOME` allocation, immutable per-target browser origin, Workspace Web Gateway, and broker identity. DataOps also provides Authorization Code with PKCE for standalone integration, atomically records an unbound target's connection owner, validates every later selection for that connection against the owner, releases only standalone bindings on verified Disconnect, exchanges single-use launch codes, issues MCP-audience tokens and scopes, authenticates each HTTP MCP request, and maps both transports onto its existing catalog, query, API, result, permission, and audit services. It adds QueryGuide storage only for genuine cross-resource guidance and implements Design B only in a later compiler project. The DataOps backend and MCP server do not import DSH packages.

DSH core requires no change. The optional `@sparkelf/dsh-dataops-integration` package is distributed from `dsh-plugins-plus` for standalone DSH, remains outside the core and default profile, and owns the connection-scoped target reference, browser OAuth, access credential, ordinary MCP header configuration, and composition of the generic MCP client. It cannot replace a bound principal inside an active connection; Disconnect rotates only a writable standalone target, and the package is not mounted in a DataOps-managed container. It contains no DataOps query identifiers, SQL execution, source routing, authorization policy, or connector logic. DSH composes `@sparkelf/dsh-chart` and `@sparkelf/dsh-query-result-analysis` through public plugin/profile extension points; these plugins consume public result capabilities and never receive DataOps transport credentials.
...
**User-managed static tokens in profile MCP config, stdio environment, or launch URLs.** Rejected because they make connection setup and revocation manual and put credentials in durable deployment inputs. The standalone plugin owns one credential-store value and supplies it to one in-memory MCP child after authorization; managed containers use broker-owned identity, and browser entry uses a one-time launch code.
...
- Each `render_chart` call names one source result and persists the complete JSON ECharts option in its direct presentation metadata or nested chart-owned dispatch block; replay never dereferences DataOps, and chart code does not join result references.
- DataOps provisions exactly one independent-`DSH_HOME` container and one immutable browser origin per user; managed containers use the trusted-internal HTTP target host, gateway, and broker identity, standalone integration uses OAuth/PKCE with a connection-scoped target binding, and neither path can rebind an active runtime to another principal; a standalone user can explicitly Disconnect before choosing again.
...
- A standalone grant ends with its bound DataOps `AuthSession`; explicit reauthorization remounts the DataOps MCP child once. Adding background refresh or periodic remount would create lifecycle complexity without improving the approved user journey.
...
Verification for this decision covers one-user-one-container allocation with independent `DSH_HOME`, per-target browser origins with isolated storage, trusted-internal HTTP bootstrap and native root routing without DSH core changes, managed gateway/broker identity without DSH-process secrets, standalone MFA login and single-use launch-code exchange, session-lifetime access and same-principal reauthorization without another login, connected target-owner and principal-mismatch rejection, standalone Disconnect followed by fresh account selection, absence of managed account-switch actions, per-request scope and Resource authorization, the absence of secrets and identity fields from model-visible calls, completed result materialization, snapshot/page/expiry authorization, export without source-operation rerun, exact eight-plus-two tool composition, and a keyless model-visible snapshot. Chart coverage must prove top-level tool registration, one-source validation, durable replay after both direct and nested Code Mode calls, keyed browser rendering, theme/resize lifecycle, and generic user-facing failure output. Batch-analysis coverage must prove bounded page reads, stable row evidence, one durable child session per batch or reduction call, complete child-session model-visible reconstruction, AgentLoop-owned provider retry, parent cancellation, completed-batch resume, and hierarchical reduction without DSH core changes. Design B must cover summary selection, detail fallback, additive and non-additive metrics, unsafe fanout rejection, dialect rendering, permission predicates, and stable versioned provenance before availability.
