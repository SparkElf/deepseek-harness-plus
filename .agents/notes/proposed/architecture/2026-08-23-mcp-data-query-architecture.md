...
DataOps binds each connected MCP session identifier to the immutable runtime principal. A refreshed or explicitly reauthorized credential for the same OIDC `sub` may continue without remounting the MCP child; a different `sub` is rejected and cannot replace credentials inside that runtime. Explicit Disconnect unmounts the standalone connection and clears its grant without releasing the target binding. MCP session identifiers, DSH conversation identifiers, and result references are not credentials. A conversation identifier may later provide audit correlation without changing authorization.
...
- Standalone access refreshes only for the same persistent target owner. A shortened refresh response marks final access expiry, and a failed refresh unmounts the MCP child. Disconnect revokes and clears the grant without releasing the target binding.
