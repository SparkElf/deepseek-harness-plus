...
DataOps binds each MCP session identifier to the immutable runtime principal. An explicitly reauthorized credential for the same OIDC `sub` may replace the prior grant and remount the MCP child once; a different `sub` is rejected and cannot replace credentials or remount that child. MCP session identifiers, DSH conversation identifiers, and result references are not credentials. A conversation identifier may later provide audit correlation without changing authorization.
