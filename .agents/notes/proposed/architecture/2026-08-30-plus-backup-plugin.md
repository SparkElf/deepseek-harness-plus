...
The Client entry owns the Settings section, local operation state, upload progress, Host progress parsing, stream-reader release before completion becomes visible, cancellation, browser download, retry, import completion, and reload action. All product copy lives in the package's typed Chinese and English dictionaries. The UI retains the accepted layout and existing semantic tokens; this migration introduces no visual redesign, new shared store, polling, or background recovery.
