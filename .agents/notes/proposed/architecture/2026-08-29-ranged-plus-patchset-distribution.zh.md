...
`dsh-plus apply`解析installed patch-package manifests，要求official source revision `0a53fb55bea101816fa226bb964ae2bed71c343b`，以显式`node-pty` build permission安装包括`dsh-better-sidebar@0.17.1`与`dsh-univer-office@0.2.12`在内的profile-owned exact external dependencies，用exact checkout的dsh CLI dependency tree及out-of-tree peers所需source workspaces共同构建profile的official package scope，把selected payloads复制进profile，配置exact npm patches，应用exact source patches，通过`build:official`构建source，并写credential-free `.dsh-plus/patchset.lock.json`。每个由`apply`持有的install都会在其package或workspace mutation之后执行`pnpm install --no-frozen-lockfile`；因此CI caller会物化新声明的requirements，而不会让pnpm校验mutation之前的lock。Target为同一个exact npm package的independent payload保留独立lock records与materialized source files，但会按patch id顺序连接成pnpm为该target接受的唯一deterministic file。Source payload先按patch id顺序在isolated Git index中replay，再以相同顺序通过Git 3-way merge应用到official worktree，因此overlapping files会在source mutation前完成组合。在worktree replay之前，`apply`会刷新内容未变的clean Git index stat项，使Docker或artifact copy后的相同blob仍可应用；真正的dirty file仍在Git patch边界明确失败。`plus` release family发现完整`@sparkelf` closure，分配一个`plus-npm-v*` tag，canonicalize packed manifest object keys，并通过manual npm workflow发布credential-free pack job两次生成且逐字比较的exact tarballs。随后manual publication audit要求22个registry entries全部public，并逐文件比较registry解包内容与该release artifact。Explicit dist-tag promotion只接受一个successful exact-tag release run，会先对其artifact重复该comparison，再移动全部22个package tags并逐个读回；仅在此流程成功后promotion unversioned installation。Desktop与tray application可以调用同一public workflow，但不是prerequisite、manifest owner、release owner或acceptance path。
...
Candidate verification从official DSH installation开始，安装npm/profile closure，运行explicit apply，并使用isolated home与`3081/3083`。Real Playwright UI acceptance通过无token或cookie的clean root URL进入Plus，先验证official branding、简体中文Session export copy、桌面Trajectory toolbar位置与下载、保留的手机Header位置及指令按钮旁的attachment picker，再覆盖Backup export/import/progress/retry/restoration、Subagent Settings persistence/execution、Document intake/history/model use、真实Univer Sheet创建/核验/合入/审阅路径、DataOps/chart、external plugins及official Turn folding。Browser diagnostics收集page errors、console errors、failed requests、CORS failures与stalls。自动验收后可将changed-input preview晋升到保留`/root/.dsh`的3080供用户显式验收；PR与发布仍须等待该验收。

Local acceptance只保留一个有界immutable build slot，其key由official revision、platform/toolchain及exact candidate tarballs组成。匹配run仅复用patched official source与credential-free profile template，以hardlink物化全新profile tree且不复制package bytes；runtime home、setting、credential、plugin-market state、Workspace、fixture、diagnostic及process仍逐轮隔离。key变化时替换旧槽，不累积build families。
