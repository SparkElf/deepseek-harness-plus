...
Electron main process 通过现有 preload IPC 提供真实的平台 home 目录、文件系统 ancestry crumbs 和目录条目。renderer 不再生成 Windows 用户名或目录列表。浏览器 preview 没有文件系统能力，因此会拒绝选择目录和安装，不再发出模拟路径或进度事件。
