...
The Windows Electron package includes the version manager renderer and the release-pinned source generator. The Windows NSIS workflow remains the packaging gate after native installer interaction. Git exact-SHA fetch is verified against the public fork, and the source reference is generated from the pull_request event's head SHA for pull-request builds or the checked-out commit for release builds, so the installer fetches a commit reachable from the fork instead of the shallow merge ref. Native Windows command failures are decoded from UTF-8 or GBK before they reach the renderer, and the root web build invokes the web package through npm so the bundled pnpm does not depend on a shell-global pnpm command. Runtime metadata version 4 stores separate Harness, candidate Harness, Supervisor, and candidate Supervisor ports; Windows starts the native Supervisor through Electron utilityProcess and an unpacked bootstrap, names the control pipe from the selected Supervisor port, and waits for the endpoint to listen before startup proceeds. The bootstrap persists the complete startup stack and the installer returns it instead of reducing child-process failures to a named-pipe ENOENT. Windows writes the status manifest in place because replacing an open file can fail with EPERM. Command sockets reset their connect timeout on every received line, the Supervisor heartbeats while waiting for Harness Web to listen, and the web startup window is 120 seconds, so slow first starts no longer time out mid-progress. The `yaml` package is unpacked beside the unpacked Supervisor sources because plain Node cannot resolve packages that live only inside app.asar, and a post-build `verify:unpacked-imports` step imports the packaged Supervisor module graph to gate packaged-only resolution failures.
