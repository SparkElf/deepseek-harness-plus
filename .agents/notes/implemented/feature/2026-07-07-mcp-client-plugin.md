...
- **Stdio process management**: disposal requests SDK close and uses the transport `onclose` signal as quiescence evidence. A missing signal logs after five seconds; an SDK close Promise that remains pending after the signal cannot wedge the Cordis fiber.
