...
The tray gains a Backup and restore entry that opens a dedicated window with two actions. Export packs the complete `dshHome` directory plus a `backup-manifest.json` marker entry into a zip chosen through the save dialog. Import validates the archive first — the marker must be present and entries with absolute paths, backslashes, or `..` segments are rejected — then stops a running Harness, extracts over `dshHome` with same-named files replaced, and restarts the Harness when it was running, so an invalid archive never leaves the runtime stopped. Missing markers and unsafe paths raise localized errors before extraction. The window warns that archives contain sensitive data such as API keys. WSL targets are rejected with a clear message because `dshHome` lives inside the distribution filesystem.
