...
Every public entry validates the domain rather than relying on one model-facing configuration path. Negative values, fractions, negative zero, non-finite values, unsafe integers, malformed stored parent depth, and derived overflow all reject. A direct `SubagentStartRequest` may omit the absolute cap to leave depth unbounded. Loader-resolved `dsh-tool-subagent` configuration instead expresses additional generations below the direct child, defaults to `0`, and adds one before sending the provider's absolute cap; zero therefore permits a child but rejects grandchildren. Explicit `'provider-managed'` omits the cap for an out-of-process provider whose deployment owns its recursion budget. [Safe subagent defaults](../bug-fix/2026-08-26-safe-subagent-defaults-and-unified-settings.md) own this product default and mapping. A numeric tool cap fails at provider mount when the provider lacks `depthLimit`.
