...
`host.openPath` is a privileged unary RPC accepted only from loopback, same-origin browser requests (same carrier guard as `host.pickDirectory`). Platform adapters open without a shell: `open` on macOS, PowerShell `Invoke-Item` on Windows, and `xdg-open` on Linux; browser-renderable documents prefer the named default browser on macOS and Linux. WSL is a separate host shape despite Node reporting `linux`: the adapter recognizes its environment or Microsoft kernel release, translates the Linux path with `wslpath -w`, resolves the standard Windows PowerShell executable with `wslpath -u`, and passes the resulting Windows/UNC path to that PowerShell handoff. The opener's platform facts and command runner are injectable for tests. URL-only read args (`web_fetch`) are not file links.
...
- Linux hosts without `xdg-open`, and WSL hosts without working Windows interop (`wslpath` plus the resolved Windows PowerShell executable), fail the RPC; the chat row stays silent while the host returns an internal error.
