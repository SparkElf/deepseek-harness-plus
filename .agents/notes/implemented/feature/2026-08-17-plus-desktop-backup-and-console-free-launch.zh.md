...
Supervisor 现在直接启动 Harness Web 和开发 watcher：在安装根目录执行 `node --import tsx/esm apps/cli/src/bin.ts web …`，使用 PATH 上的 `node`，stdio 接入 Supervisor 日志，并设置 `windowsHide: true`。这里刻意不使用 Electron 自带的 Node，因为它的 ESM 解析无法处理 pnpm 符号链接的工作区包。`pnpm` 只保留给 `pnpm run build`，其嵌套 script 链不做绕过。桌面侧所有 spawn（Supervisor 辅助命令、安装器的 git/pnpm/wsl 执行）都设置 `windowsHide: true`，GUI 进程的 console 子系统子进程不再创建窗口。Windows 进程树终止容忍 taskkill 退出码 128（进程已不存在），因为"进程已不存在"本身就满足了停止请求。托盘"退出"项显示"正在退出"提示并禁用菜单，在 8 秒宽限内尽力停止 runtime；停止失败或超时不再阻塞 `app.quit()`，退出手势永远生效且不再弹错误框。托盘复选项经 Electron 登录项设置（Windows 上为注册表 Run 项）切换开机自启动，每次重建托盘时读回勾选状态；Linux 没有登录项概念，该项在那里禁用。
