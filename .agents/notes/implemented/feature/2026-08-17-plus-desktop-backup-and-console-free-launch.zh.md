...
Supervisor 现在直接启动 Harness Web 和开发 watcher：在安装根目录执行 `node --import tsx/esm apps/cli/src/bin.ts web …`，使用 PATH 上的 `node`，stdio 接入 Supervisor 日志，并设置 `windowsHide: true`。这里刻意不使用 Electron 自带的 Node，因为它的 ESM 解析无法处理 pnpm 符号链接的工作区包。安装器管理的构建经 `hiddenBuildSteps` 直接用 `node` 执行 tsc/tsdown/vite 各阶段，因为 pnpm 嵌套生命周期的孙进程会在 Windows 上强弹可见 console 窗口；运行时 Supervisor rebuild 则调用根 `build:official` 脚本；见 [2026-08-18-hidden-build-steps](2026-08-18-hidden-build-steps.md)和[2026-08-18-client-build-environment](../architecture/2026-08-18-client-build-environment.md)。桌面侧所有 spawn（Supervisor 辅助命令、安装器的 git/pnpm/wsl 执行）都设置 `windowsHide: true`，GUI 进程的 console 子系统子进程不再创建窗口。Supervisor 无法接管配置端口时，应用弹窗给出占用者——进程名不可用时经 netstat/PowerShell 兜底给出裸 PID——确认后即结束该进程并重试启动，未命名占用不再把启动逼进死路。Windows 进程树终止容忍 taskkill 退出码 128（进程已不存在），因为"进程已不存在"本身就满足了停止请求。托盘"退出"项显示"正在退出"提示并禁用菜单，在 8 秒宽限内尽力停止 runtime；停止失败或超时不再阻塞 `app.quit()`，退出手势永远生效且不再弹错误框。安装器预填 native 安装目录：重配置时为现有 runtime 目录，新装时为`<托盘安装目录>\dsh`；目录浏览器在 Windows 上以虚拟盘符层打开，可直接选择任意盘符；端口被占用时弹窗给出占用进程名并提供"结束进程并继续"；覆盖复选框现在允许在已有托盘 runtime 上重装，先按退出宽限停止旧 runtime。托盘复选项按平台切换开机自启动：Windows——包括 runtime 目标为 WSL 的情况，因为托盘应用本体始终运行在 Windows——使用 Electron 登录项注册表项；Linux 桌面写入或删除 XDG autostart `.desktop` 条目；两者都在每次重建托盘时读回勾选状态。NSIS 安装器把新安装默认到 `%USERPROFILE%\DeepSeek Harness Plus`（每用户、可见），保留目录选择页，升级时沿用旧安装记录的目录，因为内置注册表查询在 preInit 默认值之后运行并覆盖它。每次安装写入 `run-initial-setup` 标记，应用启动时消费它并重新弹出安装向导，即使 runtime 配置已存在；完成页的运行复选项文案即"运行初始化设置"；托盘还提供"重新配置安装…"，无需重装即可重进向导。存活 Supervisor 的端口配置与当前表单不一致时会被替换：daemon 先请其 shutdown，旧版本不识别该命令时按命令行强杀，确保新 Supervisor 能绑定端口。托盘操作经气泡通知宣告开始、完成与失败，daemon 阶段文案进入托盘 tooltip，长操作不再无声。0.4.0 起 web profile 模板默认挂载 `dsh-better-sidebar`，存量安装的自有元组自动规范化到模板，新装与常规升级无需手动装插件即带 VSCode 式侧边栏。应用订阅 `window-all-closed` 并 `preventDefault`：Supervisor 以 Electron utilityProcess 运行，生命周期随应用，过去 Electron 默认的"最后窗口关闭即退出"会在安装向导关闭时把托盘、Supervisor 与 Harness Web 一起带走；现在托盘应用驻留到用户退出。
