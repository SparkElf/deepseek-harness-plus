...
**Disposal.** Dispose flips the fence, cancels any pending timer, requests close on the current client, and waits up to five seconds for its transport `onclose` signal. It then awaits the in-flight attempt and sync queue before unregistering — quiescence comes from the transport signal and owned queues, not settlement of the SDK close Promise. The reconnect timer is unref'd so a waiting backoff never holds a finishing process open.
