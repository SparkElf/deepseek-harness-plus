...
Plus不寻求对官方仓库的pull request：该仓库报告`has_pull_requests: false`，distribution只通过跟进其released revisions、并在released source已包含该behavior时retire对应patch来达成official behavior。
...
Official baseline merge会带入upstream `.github/`树以及读取它的gate，因此每个workflow在resolve前先分类。Repository-owned CI policy——runner选择、required-check aggregate、review approval、issue management，以及断言它们的`scripts/ci-workflow.spec.ts` gate——保留仓库副本；静默恢复upstream副本会加入upstream-only job（weighted approval、pull-request benchmark lane）并丢掉repository-only job，从而改变pull-request verdict。读取baseline源码路径的CI则改为跟随baseline，因为merge会重命名该路径：release与sandbox workflow跟随baseline把native package layout从`native/landlock-run`迁到`native/system`，拥有这些路径的baseline workflow名称取代仓库名称。Official baseline merge同时会取用upstream release family源码，因此每个official family都必须保留自己的package-name ownership predicate：official family只拥有`@deepseek-ai`名称并跳过并存于同一路径的Plus package，Plus family则在自身path patterns下拥有`@sparkelf`名称。丢失该predicate的family会拒绝另一authority的manifest，导致pack与verify job失败。Official baseline merge也会重定义shipped default model：rc.1把base bundle默认的`deepseek-v4-flash`换为新的multimodal `deepseek-flash`（DeepSeek-V41-Flash），同时保留全部V4 catalogue entry，因此baseline merge会改变未配置部署实际使用的模型。Plus升级需要exact official revision与fresh profile证明。未发布npm的external package保持为固定reviewed release asset。Official升级可能改变数百个package fingerprint，因此promotion policy从真实baseline与candidate profile生成，不复制旧policy。删除冗余plugin会缩小dependency graph，并把official UI extension points留给新的focused plugin。
