...
`bind()` returns a `SettingsScope<T>` selector over the mirror: it has no read path of its own, applies the namespace decode rules, and serializes `set`/`unset` field writes plus whole-section `replace` writes in one revision-fenced queue. A committed write folds its answered view back into the mirror (`acceptView`), so sibling scopes see the new revision with no re-read; the fold invalidates any older in-flight answer, and a write before the first held document reruns that read instead of publishing a partial document. A failed latest write triggers one mirror recovery read. Cross-namespace surfaces — the plugin-directory tab, the permission row (its dynamic enum lives in the namespace schema, which scopes deliberately do not carry), the models join, the agent-preset row's writability, `hasDocument`, and `canOpenDocument` — consume `ctx.settingsScope.describe()`, the shared read/fold face (`getSnapshot`/`subscribe`/`ensure`/`acceptView`).
