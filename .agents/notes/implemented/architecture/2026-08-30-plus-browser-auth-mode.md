...
The Plus profile opens through its clean root URL without a process token or browser cookie. The official `web` profile and omitted configuration retain `required`; the patch changes no capability outside Client Connection. A future non-loopback launch would widen this trust decision, so the shipped CLI continues to prevent it.
