...
Declared deployment trust and administrative controls are design inputs, not deficiencies for application code to compensate. An explicitly trusted internal network may use HTTP; external ingress uses explicitly configured HTTPS without fallback. Without a product or protocol requirement, an integration does not add short-lived token rotation, refresh machinery, application-layer encryption, locks, queues, or periodic remounts in the name of generic security. Network policy, account and session lifecycle, permission enforcement, and operator revocation own those controls while protocol-required identity and authorization checks remain enforced.

Security lifecycle choices optimize the user's complete path: one understandable authorization should remain usable until a user or administrator changes the owning session, account, permission, or connection. A mechanism that adds repeated authorization, background churn, tool interruption, or recovery states without an explicit threat or protocol requirement is rejected even when it is conventionally described as hardening.
...
- The [external plugin maintenance scheme](../process/2026-08-19-external-plugin-maintenance.md) continues to own curation pins, drift, local patches, and the separate plugin repository; this decision owns the earlier placement classification.
