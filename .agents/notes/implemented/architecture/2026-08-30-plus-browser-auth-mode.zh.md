...
Plus profile无需process token或browser cookie即可通过clean root URL打开。official `web` profile及省略配置的composition继续使用`required`；patch不改变Client Connection以外的capability。未来若允许non-loopback launch，会扩大该trust decision，因此shipped CLI继续阻止这种launch。
