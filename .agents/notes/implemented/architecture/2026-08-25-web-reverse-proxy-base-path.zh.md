...
默认外部 package `dsh-better-sidebar` 拥有 `/sidebar/api/*` HTTP carrier。Plus 通过固定版本的 pnpm patch 让该 carrier 相对 `document.baseURI` 解析；外部 release 提供同等行为后移除其中的 document-base hunk。
