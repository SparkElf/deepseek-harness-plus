...
设置备份客户端继续作为一方包，因为其归档行为随 Harness 设置和存储数据演进；它随 DSH 包家族发布，并由 Web bundle 默认挂载。移动桥接归 `dsh-plugins-plus`：其 Host 隧道、Client 设置区、移动端呈现、中继协议、配置和文档均由 `@sparkelf/dsh-mobile-bridge` 交付。Plus Web profile 钉扎并默认挂载该外部包，因为所有者把移动访问选为此发行版的一部分；更高层 profile patch 可以禁用它，上游 Harness 不因此取得其源码或发布责任。
...
- 一方包无需迁仓即可发布 npm，在确需锁步开发时保留同仓协作。缺少所有者凭据会阻塞发布，而不会授权修改 scope 或版本合同。
- Plus 可以策展或默认挂载外部包，同时由外部项目继续承担发布与安全责任。
- [外部插件维护方案](../../../notes/proposed/architecture/2026-08-19-external-plugin-maintenance.md)继续负责策展钉扎、漂移、本地补丁和独立插件仓库；本决策负责更早的归属分类。
