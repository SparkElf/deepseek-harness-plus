...
The default external `dsh-better-sidebar@0.16.1` package owns its API, upload, media, HTML preview, lazy bundle, and WebSocket carriers below `/sidebar`. Plus carries a pinned pnpm patch that resolves every client carrier against `document.baseURI`; [upstream PR #422](https://github.com/omdsh-dev/DSH-better-sidebar/pull/422) owns the external implementation and focused tests. This temporary hunk retires after that change is merged and released, and DSH adopts the containing release.
