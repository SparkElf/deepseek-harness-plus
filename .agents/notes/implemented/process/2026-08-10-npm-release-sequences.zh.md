...
### 已发布的包在消费方镜像提供它之前还不可安装

一次发布只写入一个 registry。使用镜像的消费方——大陆镜像正是本发行版用户的默认选择——读到的是同步副本，而这个副本要等镜像抓取该包之后才存在。在该抓取完成前用镜像安装，会以 tarball 的 `404` 失败，读起来像包不存在，而不是尚未就绪。

这个失败容易被误判，因为包确实存在于发布目标处：对规整 registry 执行 `npm view` 会对同一个版本返回 `200`，而消费方却装不上。因此验证一次发布，意味着**通过消费方使用的 registry** 安装已发布的版本，而不是通过发布者写入的那个。

在宣布发布之前触发镜像抓取并等待它完成。尚未缓存某个 scope 的镜像会对包文档本身返回 `404`；在把该发布视为可安装之前，确认消费方 registry 上的包文档与它的 tarball 都返回 `200`。
