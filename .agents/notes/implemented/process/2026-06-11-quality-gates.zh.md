...
本记录中的钩子/CI 对称设计已由[快速本地 Git 钩子](2026-07-22-fast-local-git-hooks.md)取代；每个拉取请求都运行完整矩阵的设计则由 [fail-open CI 影响范围选择](../testing/2026-08-22-fail-open-pr-ci-impact.md)细化，未知和高影响改动及默认分支 nightly 仍执行完整检查。
...
每条可机械检查的 AGENTS.md 承诺都有一个以非零状态退出的命令。CI 执行 fail-open 影响范围 plan 所选择的完整相关集合，而 Git 钩子将延迟预算留给可低成本发现的本地缺陷：
...
- lefthook pre-commit 执行不加载项目的 Oxlint 验证，并应用带[一次有界重试](2026-08-09-oxlint-only-fix-workflow.md)的安全修复，拒绝已暂存的空白问题并检查 vendor manifest（元数据清单）；pre-push 运行增量类型检查。每个拉取请求都会运行静态检查；完整影响范围与 nightly 运行还会执行 Node 22.19/24/26 矩阵，以及 Headless、TUI、ACP（Agent Client Protocol）、JSON-RPC、工作流和代码运行时路径的已构建应用入口约定。

## 后果

- 约定不会因 agent 更替而失效；可低成本发现的 commit/push 缺陷会在本地触发失败，相关违规会在拉取请求 CI 中触发失败，nightly 完整矩阵则检测跨范围漂移。
