...
从托管池故障中恢复只需切换受影响平台的故障切换变量（任何写者可设）加一次重跑，关键路径上没有合并。无法使用默认企业 Linux 标签的仓库还要拥有一个稳定的 `DSH_CI_HOSTED_LINUX_RUNNER` 值；它让常规运行留在可用的 GitHub-hosted 基础设施上，不引入本地runner路径。代价是每个平台都要维护第二套运行器拓扑：热备通道在每次 master 推送时都运行它们，避免故障切换目标变得陈旧；而 `ci.yml` 中的快照并发与缓存恢复分支带有一条 `selfhosted` 支路（仅 Linux），必须与托管支路保持同步。按平台拆分开关多了一个需要管理的变量，但把每个开关的影响范围限定在单个平台的作业上。
